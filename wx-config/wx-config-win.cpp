...
public:
    ...
    bool validArgs()
    {
        ...
        if(!valid)
        {
            ...
            std::cerr << "  --iso                       Output only \"iso C++\" options, ignore WX settings \n";
...
protected:
    ...
    void parseLibs(const std::string libs)
    {
        ...
        {
            addLib("aui");  // added by CA
            addLib("stc");  // added by CA, this also requires wxscintilla, see __LIB_SCINTILLA_p
            addLib("propgrid");  // added by CA
            addLib("xrc");
            addLib("qa");
            addLib("html");
            addLib("adv");
...
/*
    ...
    void process_3(Options& po, const CmdLineOptions& cl, BuildFileOptions& cfg)
    {
        ...
        /// External libs (to wxWidgets)
           ...
           if (cfg["USE_GUI"] == "1")                                                    // added by CA
               if (cfg["USE_STC"] == "1")                                                // added by CA
                   po["__LIB_SCINTILLA_p"] = addLib("wxscintilla" + po["WXDEBUGFLAG"]);  // added by CA
...
*/
    ...
    std::string getAllLibs(Options& po)
    {
        ...
        libs += po["__WXLIB_STC_p"]  + po["__LIB_SCINTILLA_p"];                       // Added by CA
...
/* CA 20131207
        ...
        The following 2 ifs are incorrect and has been removed
        whether to use debug mode is *completely* defined by the
        BUILD option taken from the generated build.cfg file
...
//  CA 20131207 added this instead
        if (cfg["BUILD"] == "debug")
            po["WXDEBUGFLAG"] = "d";
//  CA 20131207 end
        ...
        if ( (cfg["TARGET_CPU"] == "x64")  || (cl.keyValue("--wxcfg").find("64") != std::string::npos) )
            po["DIR_SUFFIX_CPU"] = "_x64";
...
//        po["cflags"] += easyMode(po["__DEBUGINFO_0"]) + easyMode(po["____DEBUGRUNTIME_2_p"]);
          po["cflags"] += po["__DEBUGINFO_0"] + po["__DEBUGINFO_1"] + easyMode(po["____DEBUGRUNTIME_2_p"]);
        }

        po["cflags"] += easyMode(po["__OPTIMIZEFLAG_4"]) + po["__NO_VC_CRTDBG_p"];
        if(!cl.keyExists("--iso")) {
           po["cflags"] +=  addDefine("__WXMSW__") + po["__WXUNIV_DEFINE_p"];
        }
        po["cflags"] += po["__DEBUG_DEFINE_p"] + po["__EXCEPTIONS_DEFINE_p"] + po["__RTTI_DEFINE_p"];
        ...
        if(!cl.keyExists("--iso")) {
           po["cflags"] += addIncludeDir(po["SETUPHDIR"]);
           po["cflags"] += addIncludeDir(po["prefix"] + "\\include");
        }
        po["cflags"] += easyMode(addFlag("/W4"));
        po["cflags"] += easyMode(addIncludeDir(".")) + po["__DLLFLAG_p"] + addDefine("_WINDOWS");
        po["cflags"] += easyMode(addIncludeDir(po["prefix"] + "\\samples")) + easyMode(addDefine("NOPCH")) + po["__RTTIFLAG_9"] + po["__EXCEPTIONSFLAG_10"];
        po["cflags"] += cfg["CPPFLAGS"] + " " + cfg["CXXFLAGS"] + " ";
